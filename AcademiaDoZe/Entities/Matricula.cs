...
namespace AcademiaDoZe.Domain.Entities
{
    public sealed class Matricula : Entity
    {
        ...
        {
            return Criar(0, alunoMatricula, plano, dataInicio, dataFim, objetivo, restricoesMedicas, laudoMedico, observacoesRestricoes);
        }
        public static Matricula Criar(
            int id,
            Aluno alunoMatricula,
            ...
            string observacoesRestricoes = "")
        {
            if (alunoMatricula is null) throw new DomainException("ALUNO_INVALIDO");

            if (alunoMatricula.DataNascimento > DateOnly.FromDateTime(DateTime.Today.AddYears(-16)) && laudoMedico is null)
                throw new DomainException("MENOR16_LAUDO_OBRIGATORIO");

            if (!Enum.IsDefined(typeof(EMatriculaPlano), plano))
                throw new DomainException("PLANO_INVALIDO");

            if (dataInicio == default) throw new DomainException("DATA_INICIO_OBRIGATORIO");
            if (dataFim == default) throw new DomainException("DATA_FIM_OBRIGATORIO");
            if (dataFim <= dataInicio) throw new DomainException("DATA_FIM_MENOR_OU_IGUAL_INICIO");

            if (NormalizadoService.TextoVazioOuNulo(objetivo))
                throw new DomainException("OBJETIVO_OBRIGATORIO");
            objetivo = NormalizadoService.LimparEspacos(objetivo);

            if (!Enum.IsDefined(typeof(EMatriculaRestricoes), restricoesMedicas))
                throw new DomainException("RESTRICOES_INVALIDAS");

            if (restricoesMedicas != EMatriculaRestricoes.None && laudoMedico is null)
