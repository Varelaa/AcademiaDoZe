...
namespace AcademiaDoZe.Domain.Entities
{
    public class Colaborador : Pessoa
    {
        ...
        public static Colaborador Criar(
            ...
            EColaboradorVinculo vinculo)
        {
            return Criar(0, nome, cpf, dataNascimento, telefone, email, endereco, numero, complemento, senha, foto, dataAdmissao, tipo, vinculo);
        }
            ...
            EColaboradorVinculo vinculo)
        {
            ...
            if (endereco is null) throw new DomainException("LOGRADOURO_OBRIGATORIO");
            ...
            if (dataAdmissao > DateOnly.FromDateTime(DateTime.Today))
                throw new DomainException("DATA_ADMISSAO_MAIOR_ATUAL");

            if (!Enum.IsDefined(typeof(EColaboradorTipo), tipo))
                throw new DomainException("TIPO_COLABORADOR_INVALIDO");

            if (!Enum.IsDefined(typeof(EColaboradorVinculo), vinculo))
                throw new DomainException("VINCULO_COLABORADOR_INVALIDO");

            if (tipo == EColaboradorTipo.Administrador && vinculo == EColaboradorVinculo.CLT)
                throw new DomainException("ADMINISTRADOR_CLT_INVALIDO");
